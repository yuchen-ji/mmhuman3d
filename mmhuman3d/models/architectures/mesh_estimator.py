...
class BodyModelEstimator(BaseArchitecture, metaclass=ABCMeta):
    ...
    def compute_losses(self, predictions: dict, targets: dict):
        ...
        if self.loss_heatmap2d is not None:
            losses["heatmap2d_loss"] = self.compute_heatmap2d_loss(
                pred_heatmap2d, gt_heatmap2d, gt_heatmap2d_conf
            )
...
class ImageBodyModelEstimator(BodyModelEstimator):
    ...
    def forward_test(self, img: torch.Tensor, img_metas: dict, **kwargs):
        ...
        # By Yuchen, 23.02.27, if feature is list, which causes by multi-task branch
        if isinstance(features, list):
            has_heatmaps = True
            heatmaps = features[1]  # (num_person, 54, 56, 56)
            features = features[0]
        ...
        # 如果有分支预测2d关键点热力图
        if has_heatmaps:
            all_preds["heatmaps_2d"] = heatmaps.cpu().numpy()
